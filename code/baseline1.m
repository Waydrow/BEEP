% AAAI2015
% PreWhether
function  result = baseline1(trainX,trainY,testX,testY, istrain)
time0 = cputime;
...
[hpara,~] = fmincon(@(para) likelihood_b1_1(trainX(find(trainY(:,1)==1),:),para),rand(6,1),[],[],[],[],[0,0,0,0,-200,0],[200,200,200,200,200,200], [] ,options);
[npara,~] = fmincon(@(para) likelihood_b1_1(trainX(find(trainY(:,1)==0),:),para),rand(6,1),[],[],[],[],[0,0,0,0,-200,0],[200,200,200,200,200,200], [],options);
...
fid = fopen('parameters_t_pre.txt', 'r');
    hpara = fscanf(fid, '%f', 6);
    npara = fscanf(fid, '%f', 6);
    fclose(fid);
end

po_c(:,1) = pc(1) .* betapdf(max(testX(:,1), 0.001),hpara(1),hpara(2)) .* gampdf(max(testX(:,2), 0.001),hpara(3),hpara(4)) .* normpdf(max(testX(:,3), 0.001),hpara(5),hpara(6));
po_c(:,2) = pc(2) .* betapdf(max(testX(:,1), 0.001),npara(1),npara(2)) .* gampdf(max(testX(:,2), 0.001),npara(3),npara(4)) .* max(normpdf(max(testX(:,3), 0.001),npara(5),npara(6)), 0.001);
